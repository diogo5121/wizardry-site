...
const CadastroNovoId = () => {
        ...
        axios.post(`${ipApi}/api/cadastro`, data, { headers })
            .then(response => {
                ...
                const data = {
                    ...
                    email: email,
                    numero: telefone,
                    referencialid: sanitizeInput(id)
                };
    ...
    const validarInput = () => {
        ...
        //codigoafiliado
        const codigoafiliadoValido = allusers.filter(user => user.codigoafiliado === id).length === 1
                ...
                <Box display={'flex'} flexDirection={'column'} bg="#edf3f8" borderRadius={20} p={7} justifyContent={'center'} alignItems={'center'}>
                    ...
                    <Stack spacing={2} w={{ base: 300, md: 600 }}>
                        ...
                        <InputGroup size='sm' display={'flex'} flexDirection={'column'} m={1} shadow={'sm'}>
                            ...
                            <Input placeholder='(Opcional)' size={'lg'} type='text' value={sanitizeInput(id)} isDisabled={true}/>
                            {allusers.filter(user => user.codigoafiliado === id).length === 0 && id.length > 0 && (
